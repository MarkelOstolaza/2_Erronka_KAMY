...
public class Data {
    ...
    private static void populateSchedule(Map<String, List<Show>> schedule) {
        ...
        for (Map.Entry<String, Boolean> weekday : WEEKDAY_MAP.entrySet()) {
            boolean isOpen = weekday.getValue();
            ...
            for (int showNum = 1; showNum <= maxShows; ++showNum) {
                ...
                String showId = String.format("D%dS%s", dayNumber, leadingZero);
            ...
            SCHEDULE.put(weekday.getKey(), showList);

            logger.info(String.format("Added \"%s\" to the schedule as day %d.", weekday.getKey(), dayNumber));

            openDays++;
            dayNumber++;
        }

        logger.info(String.format("Successfully generated schedules for %d open weekdays.", openDays));
    }
